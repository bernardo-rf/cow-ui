            ...
            <div class="container main">
                <div class="columns is-centered is-full-height is-vcentered">
                    <div class="column has-text-centered pt-4">
                                    ...
                                    <div class="card-content">
                                        ...
                                        <validation-observer ref="observer" v-slot="{ handleSubmit }">
                                            <form @submit.prevent="handleSubmit(submit)">
                                                <validation-provider rules="required|min:2|alpha_spaces|max:15" name="Name"
                                                    v-slot="validationContext">
                                                    <b-field label="Name" :type="getValidationState(validationContext)"
                                                        :message="validationContext.errors[0]">
                                                        <b-input rounded v-model="name"></b-input>
                                                    </b-field>
                                                </validation-provider>
                                                <validation-provider rules="required|email|max:50" name="Email"
                                                    v-slot="validationContext">
                                                    <b-field label="Email" :type="getValidationState(validationContext)"
                                                        :message="validationContext.errors[0]">
                                                        <b-input rounded v-model="email"></b-input>
                                                    </b-field>
                                                </validation-provider>
                                                <validation-provider vid="password"
                                                    rules="required|capitalLetter|oneNumber|min:8|max:30" name="Password"
                                                    v-slot="validationContext">
                                                    <b-field label="Password" :type="getValidationState(validationContext)"
                                                        :message="validationContext.errors[0]">
                                                        <b-input rounded type="password" v-model="password"
                                                            password-reveal></b-input>
                                                    </b-field>
                                                </validation-provider>
                                                <validation-provider vid="confirmation" rules="required|confirmed:password"
                                                    name="Password Confirmation" v-slot="validationContext">
                                                    <b-field label="Password Confirmation"
                                                        :type="getValidationState(validationContext)"
                                                        :message="validationContext.errors[0]">
                                                        <b-input rounded type="password" v-model="confirmation"
                                                            password-reveal></b-input>
                                                    </b-field>
                                                </validation-provider>
                                                <validation-provider rules="required" name="User Type"
                                                    v-slot="validationContext">
                                                    <b-field rounded label="User Type"
                                                        :type="getValidationState(validationContext)"
                                                        :message="validationContext.errors[0]">
                                                        <b-select v-model="userType" rounded expanded
                                                            placeholder="Select a User Type">
                                                            <option v-for="userType in userTypes"
                                                                :value="userType.idUserType" :key="userType.idUserType">
                                                                {{ userType.typeDescription }}
                                                            </option>
                                                        </b-select>
                                                    </b-field>
                                                </validation-provider>
                                                <b-button class="mt-3" type="is-primary" expanded rounded
                                                    native-type="submit" :loading="isLoading">Sign Up</b-button>
                                            </form>
                                        </validation-observer>
                                    </div>
                                </div>
                        ...
                        <div class="columns">
                            <div class="column">
                                <b-button type="is-text" tag="router-link" to="/login">Already have an account? Sign
                                    In.</b-button>
                            </div>
                        </div>
                        <div class="columns">
